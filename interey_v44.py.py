...
DEFAULT_BACKLOG_FILES = [
    "Proyectos en ejecucion.csv",
    "Backlog Proyectos.csv",
    "Network Solutions Sistema de administración. (16).csv",
]
...
def load_backlog(uploaded_file):
    ...
    A diferencia de ventas, el backlog NO se acumula con el archivo anterior:
    cada carga mensual sustituye el snapshot, para que los proyectos ya facturados
    desaparezcan correctamente.
    """
    if uploaded_file is not None:
        try:
    ...
    missing = dt.isna()
    if missing.any():
        dt.loc[missing] = pd.to_datetime(raw.loc[missing], format="%d/%m/%Y", errors="coerce")
    missing = dt.isna()
    if missing.any():
        dt.loc[missing] = pd.to_datetime(raw.loc[missing], dayfirst=True, errors="coerce")
    df["Fecha"] = dt
    df = df[df["Fecha"].notna()].copy()

    if "Status" in df.columns:
        status = df["Status"].fillna("").astype(str).str.strip().str.upper()
        accepted = status.isin(["ACEPTADA", "ACEPTADO", "APROBADA", "APROBADO"])
        if accepted.any():
            df = df[accepted].copy()
    ...
    df["Moneda"] = df.get("Moneda", "MXN")
    df["Moneda"] = df["Moneda"].fillna("MXN").astype(str).str.upper()
    df["TC"] = pd.to_numeric(df.get("TC", 1), errors="coerce").fillna(1.0)
    df["Tipo_Cambio_Aplicado"] = df.apply(lambda r: r["TC"] if r["Moneda"] == "USD" else 1.0, axis=1)
    df["Importe_Pendiente_MXN"] = pd.to_numeric(df.get("Cotizado cliente", 0), errors="coerce").fillna(0) * df["Tipo_Cambio_Aplicado"]

    today = pd.Timestamp.today().normalize()
    df["Dias_Abiertos"] = (today - df["Fecha"].dt.normalize()).dt.days.clip(lower=0)
    df["Año_OC"] = df["Fecha"].dt.year
    df["Mes_Num_OC"] = df["Fecha"].dt.month
    df["Mes_OC"] = df["Mes_Num_OC"].map(MONTHS_ES)
    df["Periodo_OC"] = df["Fecha"].dt.to_period("M").astype(str)

    def bucket(days):
        if days <= 30: return "🟢 En tiempo"
        if days <= 60: return "🟡 Seguimiento"
        if days <= 90: return "🟠 Riesgo"
        return "🔴 Crítico"
    df["Antigüedad"] = df["Dias_Abiertos"].apply(bucket)
    return df.drop_duplicates(subset=["Id"], keep="last") if "Id" in df.columns else df.drop_duplicates(keep="last")
...
st.sidebar.caption("Si no subes archivos, el dashboard intentará usar los archivos maestros desde la misma carpeta. El archivo de proyectos en ejecución es un snapshot vigente y debe reemplazarse completo cada mes.")
...
data_max_date = latest_data_date(projects_year, store_year, backlog)
...
view_options = ["🏠 Resumen Ejecutivo", "📊 Consolidado", "🏗️ Proyectos", "📋 Ingresos comprometidos", "🏪 Tienda"]
view_map = {
    "🏠 Resumen Ejecutivo": "Resumen Ejecutivo",
    "📊 Consolidado": "Consolidado",
    "🏗️ Proyectos": "Proyectos",
    "📋 Ingresos comprometidos": "Backlog",
    "🏪 Tienda": "Tienda",
}
...
if view_selected == "Resumen Ejecutivo":
    ...
    st.markdown('<div class="section-title">Backlog Ejecutivo INTEREY</div>', unsafe_allow_html=True)
    trend_note("Incluye proyectos con Orden de Compra aprobada pendientes de facturación. La antigüedad se calcula desde la fecha registrada en el archivo.")

    if backlog.empty:
        st.info("No se cargó el archivo vigente de proyectos en ejecución. Sube el CSV completo desde la barra lateral.")
    else:
        total_backlog = backlog["Importe_Pendiente_MXN"].sum()
        total_projects = len(backlog)
        avg_age = backlog["Dias_Abiertos"].mean() if total_projects else 0
        oldest = backlog.sort_values("Dias_Abiertos", ascending=False).iloc[0]
        critical = backlog[backlog["Dias_Abiertos"] > 90]

        b1,b2,b3,b4 = st.columns(4)
        with b1: st.markdown(card("Ingresos comprometidos", fmt_money(total_backlog), "OC aprobadas pendientes de facturar"), unsafe_allow_html=True)
        with b2: st.markdown(card("Proyectos abiertos", f"{total_projects:,}", "Snapshot vigente del archivo mensual", "gray"), unsafe_allow_html=True)
        with b3: st.markdown(card("Antigüedad promedio", f"{avg_age:,.0f} días", "Desde la fecha de OC", "yellow" if avg_age > 60 else "green"), unsafe_allow_html=True)
        with b4: st.markdown(card("Proyecto más antiguo", f"{int(oldest['Dias_Abiertos']):,} días", str(oldest.get("Cliente", "Sin cliente")), "red" if oldest["Dias_Abiertos"] > 90 else "yellow"), unsafe_allow_html=True)

        buckets = pd.DataFrame([
            {"Rango":"🟢 0–30 días", "Proyectos":int((backlog["Dias_Abiertos"] <= 30).sum()), "Importe":backlog.loc[backlog["Dias_Abiertos"] <= 30, "Importe_Pendiente_MXN"].sum()},
            {"Rango":"🟡 31–60 días", "Proyectos":int(((backlog["Dias_Abiertos"] > 30) & (backlog["Dias_Abiertos"] <= 60)).sum()), "Importe":backlog.loc[(backlog["Dias_Abiertos"] > 30) & (backlog["Dias_Abiertos"] <= 60), "Importe_Pendiente_MXN"].sum()},
            {"Rango":"🟠 61–90 días", "Proyectos":int(((backlog["Dias_Abiertos"] > 60) & (backlog["Dias_Abiertos"] <= 90)).sum()), "Importe":backlog.loc[(backlog["Dias_Abiertos"] > 60) & (backlog["Dias_Abiertos"] <= 90), "Importe_Pendiente_MXN"].sum()},
            {"Rango":"🔴 Más de 90 días", "Proyectos":int((backlog["Dias_Abiertos"] > 90).sum()), "Importe":backlog.loc[backlog["Dias_Abiertos"] > 90, "Importe_Pendiente_MXN"].sum()},
        ])
        premium_simple_table(buckets, "Distribución por antigüedad", "Concentración actual del backlog por días transcurridos desde la OC.", columns=[("Rango","Antigüedad","text"),("Proyectos","Proyectos","number"),("Importe","Importe pendiente","money")], row_class_fn=lambda row, idx: "critical-row" if "Más de 90" in str(row.get("Rango","")) else ("attention-row" if "61–90" in str(row.get("Rango","")) else ("warn-row" if "31–60" in str(row.get("Rango","")) else "highlight-row")))

        monthly_backlog = backlog.groupby(["Año_OC","Mes_Num_OC","Periodo_OC"], as_index=False).agg(Proyectos=("Id","count") if "Id" in backlog.columns else ("Cliente","size"), Importe_Pendiente_MXN=("Importe_Pendiente_MXN","sum"))
        fig_backlog = px.bar(monthly_backlog, x="Periodo_OC", y="Importe_Pendiente_MXN", text="Proyectos", title="Backlog abierto por mes de recepción de OC", labels={"Periodo_OC":"Mes de OC","Importe_Pendiente_MXN":"Importe pendiente MXN","Proyectos":"Proyectos"})
        fig_backlog.update_layout(height=410, margin=dict(l=20,r=20,t=60,b=40), xaxis_tickangle=-35)
        st.plotly_chart(fig_backlog, use_container_width=True)

        table = backlog.sort_values(["Dias_Abiertos","Importe_Pendiente_MXN"], ascending=[False,False]).copy()
        table["Fecha_OC"] = table["Fecha"].dt.strftime("%d/%m/%Y")
        table["Proyecto"] = table.get("Descripcion", "")
        premium_simple_table(table, "Proyectos en ejecución con OC", "Ordenada del proyecto más antiguo al más reciente. Los colores permiten detectar inmediatamente atrasos y capital pendiente de facturación.", columns=[("Antigüedad","Semáforo","text"),("Cliente","Cliente","text"),("Proyecto","Proyecto","text"),("Promotor","Responsable","text"),("Fecha_OC","Fecha OC","text"),("Dias_Abiertos","Días abiertos","number"),("Importe_Pendiente_MXN","Importe pendiente","money")], row_class_fn=lambda row, idx: "critical-row" if float(row.get("Dias_Abiertos",0)) > 90 else ("attention-row" if float(row.get("Dias_Abiertos",0)) > 60 else ("warn-row" if float(row.get("Dias_Abiertos",0)) > 30 else "highlight-row")))

        if not critical.empty:
            st.error(f"Atención: {len(critical)} proyecto(s) superan 90 días abiertos y representan {fmt_money(critical['Importe_Pendiente_MXN'].sum())} pendientes de facturación.")
        else:
            st.success("Backlog sano: no existen proyectos con más de 90 días abiertos.")
...
with st.expander("Auditoría avanzada de datos filtrados"):
    ...
    if view_selected == "Backlog":
        cols = [c for c in ["Id","Fecha","Promotor","Cliente","Descripcion","Moneda","TC","Cotizado cliente","Importe_Pendiente_MXN","Dias_Abiertos","Antigüedad"] if c in backlog.columns]
...
with st.expander("ℹ️ Información metodológica"):
    ...
    - El archivo de **proyectos en ejecución** es un snapshot vigente: debe reemplazarse completo cada mes y no se acumula con el anterior.
    - La antigüedad del backlog se calcula desde la **fecha de OC** registrada en el CSV.
    """)

st.caption("Versión v50 · Backlog Ejecutivo con OC · Semáforo de antigüedad · Comparativo histórico por ingeniero · Radar INTEREY 3.0.")
